...
if ($synchronizeUsers) {
    $maarchUsers = \User\models\UserModel::get(['select' => ['id', 'LOWER(user_id) as user_id', 'firstname', 'lastname', 'phone', 'mail', 'status', 'mode']]);  // EDISSYUM - NCH01 Forcage des user_id en lowercase pour la synchro LDAP
...
function synchronizeUsers(array $ldapUsers, array $maarchUsers)
{
    ...
    foreach ($maarchUsers as $maarchUser) {
        $maarchUsersLogin[strtolower($maarchUser['user_id'])] = $maarchUser; // EDISSYUM - NCH01 Forcage des user_id en lowercase pour la synchro LDAP
    }
    $ldapUsersLogin = [];
    foreach ($ldapUsers as $ldapUser) {
        $ldapUsersLogin[strtolower($ldapUser['user_id'])] = $ldapUser; // EDISSYUM - NCH01 Forcage des user_id en lowercase pour la synchro LDAP
    }

    foreach ($ldapUsers as $user) {
        $user['user_id'] = strtolower($user['user_id']); // EDISSYUM - NCH01 Forcage des user_id en lowercase pour la synchro LDAP
        ...
        if (!empty($maarchUsersLogin[$user['userId']]) && $maarchUsersLogin[$user['userId']]['status'] != 'DEL') {
            ...
            if (!empty($control['errors'])) {
                writeLog(['message' => "[ERROR] Control create user [{$user['user_id']}] failed : {$control['errors']}"]); // EDISSYUM - NCH01 Forcage des user_id en lowercase pour la synchro LDAP
                continue;
            }
