...
try {
    ...
    if (!in_array($configRemoteSignatoryBook['id'], ['maarchParapheur', 'xParaph', 'fastParapheur', 'iParapheur', 'ixbus', 'blueway', 'pastell'])) { // EDISSYUM - PYB01 Ajout du connecteur Blueway, Ajout du connecteur Pastell| added blueway, pastell
...
} elseif ($configRemoteSignatoryBook['id'] == 'blueway') { // EDISSYUM - PYB01 Ajout du connecteur Blueway | rajout elseif
    $retrievedMails = \ExternalSignatoryBook\controllers\BluewayController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'noVersion']);
} elseif ($configRemoteSignatoryBook['id'] == 'pastell') { // EDISSYUM - PYB01 Ajout du connecteur Pastell | rajout elseif
    $retrievedMails = \ExternalSignatoryBook\controllers\PastellController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'noVersion']);
}
...
if (!empty($idsToRetrieve['resLetterbox'])) {
    if ($configRemoteSignatoryBook['id'] == 'maarchParapheur') {
        ...
        $retrievedLetterboxMails = \ExternalSignatoryBook\controllers\BluewayController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'resLetterbox']);
    } elseif ($configRemoteSignatoryBook['id'] == 'pastell') { // EDISSYUM - PYB01 Ajout du connecteur Pastell | rajout elseif
        $retrievedLetterboxMails = \ExternalSignatoryBook\controllers\PastellController::retrieveSignedMails(['config' => $configRemoteSignatoryBook, 'idsToRetrieve' => $idsToRetrieve, 'version' => 'resLetterbox']);
    }
...
foreach ($retrievedMails['noVersion'] as $resId => $value) {
    ...
    if ($value['status'] == 'validated') {
        ...
        \Entity\models\ListInstanceModel::update([
            ...
            'data' => [$value['res_id_master'], 'VISA_CIRCUIT', 'sign']
        ]);
        // END EDISSYUM - NCH01
