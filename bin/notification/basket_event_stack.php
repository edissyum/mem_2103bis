...
foreach ($baskets as $basket) {
    ...
    foreach ($groups as $group) {
        ...
        foreach ($users as $userToNotify) {
            // EDISSYUM - NCH01 Fix notifications des bannettes désactivées
            // Mettre toutes les lignes contenu dans le foreach ($users as $userToNotify) dans le nouveau foreach ($allGroups as $group)
            $allGroups = \SrcCore\models\DatabaseModel::select([
                'select'    => ['group_id'],
                'table'     => ['usergroup_content'],
                'where'     => ['user_id = ?'],
                'data'      => [$userToNotify['id']]
            ]);
            foreach ($allGroups as $group) {
                $preference = \User\models\UserBasketPreferenceModel::get([
                    'select'    => ['*'],
                    'where'     => ['user_serial_id = ?', 'group_serial_id = ?', 'basket_id = ?'],
                    'data'      => [$userToNotify['id'], $group['group_id'], $basket['basket_id']]
                ]);
                var_dump($preference);
                if (!empty($preference)) {
