...
class ReconciliationController
{
    ...
    public static function getWs($aArgs)
    {
        ...
        if ($loadedXml) {
            ...
            $closed_status           = $reconciliationConfig->closed_status;
            ...
            if ($close_incoming == 'true') {
                $res_status = ResModel::getById([
                    'select' => ['status'],
                    'resId' => $res_id_master
                ]);

                $set = ['status' => $closed_status];
                $message = 'modification du status';
                if ($res_status['status'] != 'END' && $closed_status == 'END' || $closed_status == '') {
                    $message = 'clôture';
                    $set = ['status' => 'END', 'closing_date' => 'CURRENT_TIMESTAMP'];
                }
                ...
                HistoryController::add([
                    ...
                    'eventType' => 'ACTION#',
                    'info'      => 'Réconciliation : ' . $message . ' du document cible dans le cadre de la réconciliation automatique',
                    'moduleId'  => 'reconciliation',
