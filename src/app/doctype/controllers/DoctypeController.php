...
use SrcCore\models\DatabaseModel; // EDISSYUM - NCH01 Module formulaire SVE

class DoctypeController
{

    // EDISSYUM - NCH01 Module formulaire SVE
    public function getSVEDoctypes(Request $request, Response $response)
    {
        $doctypeType = $request->getParams()[0];
        $stmt = DatabaseModel::select([
            'select'    => ['doctypes.type_id', 'doctypes.description', 'doctypes_second_level_label as sous_chemise'],
            'table'     => ['doctypes', 'doctypes_second_level', 'doctypes_first_level'],
            'left_join' => ['doctypes.doctypes_second_level_id = doctypes_second_level.doctypes_second_level_id', 'doctypes_second_level.doctypes_first_level_id = doctypes_first_level.doctypes_first_level_id'],
            'where'     => ['doctypes_first_level.doctypes_first_level_label = ?'],
            'data'      => [$doctypeType]
        ]);

        return $response->withJson($stmt);
    }
    // END EDISSYUM - NCH01 SVE
