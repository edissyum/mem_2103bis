...
class IParapheurController
{
    ...
    {
        $option = [
            //CURLOPT_SSLCERT         => $config['data']['certPath'], // EDISSYUM - PYB01 Fix Iparapheur sans certificat | Commenter cette ligne
            CURLOPT_SSLCERTTYPE     => $config['data']['certType'],
            CURLOPT_SSL_VERIFYPEER  => 'false',
            CURLOPT_USERPWD         => $config['data']['userId'] . ':' . $config['data']['password'],
        ];
        // EDISSYUM - PYB01 Fix Iparapheur sans certificat
        if (!empty($config['data']['certPath'])) {
            $option[CURLOPT_SSLCERT] = $config['data']['certPath'];
        }
        // END EDISSYUM - PYB01
