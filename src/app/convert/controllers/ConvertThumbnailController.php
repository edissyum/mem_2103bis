...
class ConvertThumbnailController
{
    public static function convert(array $aArgs)
    {
        ...
        if ($aArgs['type'] == 'resource') {
            ...
            // EDISSYUM - NCH01 fix de la génération de thumbnails à la volée
            else if (empty($convertedDocument)) {
                $convertedDocument = ResModel::getById([
                    'resId' => $aArgs['resId'],
                    'select' => ['docserver_id', 'path', 'filename', 'format', 'fingerprint']
                ]);
            }
            // END EDISSYUM - NCH01
        } else {
        ...
        if (in_array($ext, ['maarch', 'html'])) {
            ...
            // EDISSYUM - NCH01 Fix pour la génération des thumbnails depuis un fichier HTML
            exec('whereis xvfb-run', $outputWhereIs);
            ...
            // END EDISSYUM - NCH01 Fix pour la génération des thumbnails depuis un fichier HTML
        } else {
            $size = '750x900';
            ...
            // EDISSYUM - PYB01 Fix pour permettre à l'utilisateur apache de lancer la commande convert depuis OnlyOffice
            ...
            $apache_tmp_dir = sys_get_temp_dir ();
            $command = "HOME={$apache_tmp_dir} convert -thumbnail {$size} -background white -alpha remove "
                . escapeshellarg($pathToDocument) . '[0] ' . escapeshellarg("{$tmpPath}{$fileNameOnTmp}.png");
            // END EDISSYUM - PYB01
        }
