    ...
    public function checkExternalSignatoryBook(Request $request, Response $response, array $aArgs)
    {
        ...
        if (!empty($loadedXml)) {
            ...
            if ($signatureBookEnabled == 'ixbus') {
                $additionalsInfos['ixbus'] = IxbusController::getInitializeDatas($config, $data['resources'][0]); // EDISSYUM - NCH01 IXBUS : Selection automatique du modèle de circuit | Ajout de $data['resources'][0]
            }

            // EDISSYUM - PYB01 Ajout du connecteur Blueway
            if ($signatureBookEnabled == 'blueway') {
                $res = [];
                $resCustomFields = ResModel::getById([
                    'select'   => ['custom_fields'],
                    'resId'    => $data['resources'][0]
                ]);
                $res['custom_fields'] = json_decode($resCustomFields['custom_fields'], true);

                $listInstance = ListInstanceModel::get([
                    'select'  => ['listinstance_id', 'item_id', 'item_mode'],
                    'where'   => ['res_id = ?', 'item_mode in (?)'],
                    'data'    => [$data['resources'][0], ['dest', 'sign']],
                    'orderBy' => ['listinstance_id desc'],
                ]);

                foreach($listInstance as $item) {
                    if($item['item_mode'] == 'dest')
                        $res['initiatior'] = $user = UserModel::getById(['id' => $item['item_id'], 'select' => ['user_id', 'firstname', 'lastname']]);
                    if($item['item_mode'] == 'sign')
                        $res['signatory'] = $user = UserModel::getById(['id' => $item['item_id'], 'select' => ['user_id', 'firstname', 'lastname']]);
                }
                $additionalsInfos['blueway'] = BluewayController::getInitializeDatas($config, $res);
            }
            // END EDISSYUM - PYB01

            $availableResources = [];
            if (in_array($signatureBookEnabled, ['maarchParapheur', 'fastParapheur', 'iParapheur', 'ixbus', 'blueway', 'pastell'])) { // EDISSYUM - PYB01 Ajout du connecteur Blueway, Ajout du connecteur Pastell| added blueway, pastell
