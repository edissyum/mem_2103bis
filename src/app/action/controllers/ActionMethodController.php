...
use Slim\Http\Request; // EDISSYUM - NCH01 Rajout de l'export PESv2 dans la recherche
use Slim\Http\Response; // EDISSYUM - NCH01 Rajout de l'export PESv2 dans la recherche
use ZipArchive; // EDISSYUM - NCH01 Rajout de l'export PESv2 dans la recherche
...
class ActionMethodController
{
    ...
    const COMPONENTS_ACTIONS = [
        ...
        'exportPESV2Action'                         => 'exportPESV2', // EDISSYUM - NCH01 Module export PESV2
    ];
    ...
    public static function reconcile(array $args)
    {
        ...
        // EDISSYUM - NCH01 Fix Réconciliation manuelle non fonctionnelle si le document cible n'a pas de doc
...
//        }
    ...
    public static function pesv2(Request $request, Response $response) {
        $body = $request->getParsedBody();
        ValidatorModel::notEmpty($body, ['resources']);
        ValidatorModel::arrayType($body, ['resources']);
        $res = self::exportPESV2($body);
        if ($res['errors'])
            return $response->withStatus(400)->withJson(['errors' => $res['errors'][0]]);
    }
    // END EDISSYUM - NCH01

    // NCH01 - EDISSYUM Module Export PESV2
    public static function PESV2Log($message, $loggingEnabled) {
    ...
    public static function exportPESV2(array $args)
    {
        self::PESV2Log('Export PESV2 started', $loggingEnabled);
        ...
        $nameArr        = array();
        $statusArr      = array();
        $authorizedAttachmentTypes = '';
        $attachmentSuffix = '';

        $loadedXml = CoreConfigModel::getXmlLoaded(['path' => 'apps/maarch_entreprise/xml/file_export.xml']);
        if ($loadedXml){
            foreach ($loadedXml -> DESTINATION as $arrayConfig){
                $path = (string) $arrayConfig -> path;
            }
            foreach ($loadedXml -> NAME as $nameTag) {
            ...
            foreach ($loadedXml -> EXTENSION as $extensionTag) {
            ...
            foreach ($loadedXml -> SEPARATOR as $separatorTag){
            ...
            foreach ($loadedXml -> ATTACHMENTS as $attachmentType){
                $authorizedAttachmentTypes = (string) $attachmentType -> TYPES;
            }
            foreach ($loadedXml -> ATTACHMENTS as $attachmentSuffix){
                $attachmentSuffix = (string) $attachmentSuffix -> SUFFIX;
            }
            foreach ($loadedXml -> STATUS as $statusTag) {
                $statusArr['in'] = (string) $statusTag -> IN;
                $statusArr['out'] = (string) $statusTag -> OUT;
            }
            $exportToZip = (string) $loadedXml -> EXPORT_ZIP -> ENABLED;
            $loggingEnabled = (string) $loadedXml -> LOGGING;
            $zipFilenameTimestamp = (string) $loadedXml -> EXPORT_ZIP -> DATETIMESTAMP;

            $status_after_action = '_NOSTATUS_';
            $actions = ActionModel::get();
            foreach ($actions as $action)
                if ($action['action_page'] == 'exportPESV2Action')
                    $status_after_action = $action['id_status'];

            $table = 'res_letterbox';
            $tableAttachment = 'res_attachments';
            $view = 'res_view_letterbox';
            if(!is_writable($path)){
                return ['errors' => ['[exportPESV2] ' . _PATH_ERROR]];
            }
            $listOfFiles = [];
            for ($i=0; $i<count($args['resources']);$i++) {
                self::PESV2Log('-------------------------------------------------', $loggingEnabled);
                if (!self::check_status($args['resources'][$i], $table, $statusArr) && !$args['fromSearch']) {
                    return ['errors' => ['[exportPESV2] ' . _STATUS_ERROR]];
                }
                // begin document export
                $resource = DatabaseModel::select([
                    'select'    => ['res_letterbox.docserver_id', 'path_template', "replace(path,'#','/') as path", 'filename'],
                    'table'     => ['res_letterbox', 'docservers'],
                    'left_join' => ['res_letterbox.docserver_id = docservers.docserver_id'],
                    'where'     => ['res_id = ?', "lower(format) = 'pdf'"],
                    'data'      => [$args['resources'][$i]]
                ])[0];  // resource of the exported document

                self::PESV2Log('--- Processing PESV2 for res_id : ' . $args['resources'][$i] . ' ----', $loggingEnabled);

                if($resource) {
                    self::PESV2Log('---- Document data : ' . json_encode($resource) . ' ----', $loggingEnabled);
                    $name = self::generate_name($args['resources'][$i], $view, NULL, '', NULL, $globality);
                    $finalName = $path . '' . $name;
                    $tmpFilePath = $resource['path_template'] . $resource['path'] . $resource['filename'];
                    if(copy($tmpFilePath, $finalName)) {
                        $time = strtotime(self::retrieve_creation_date($args['resources'][$i], $table));  // get the creation date of the exported document
                        touch($finalName, $time);  // and modify the creation date of the exported file with it to keep track of the document date.
                        if ($exportToZip == 'true') array_push($listOfFiles, $finalName);
                        // begin attachment export
                        $listAttachments = self::retrieve_attachments($args['resources'][$i], $tableAttachment, $authorizedAttachmentTypes);
                        if(!empty($listAttachments)) {
                            $attachmentNumber = 1;
                            foreach($listAttachments as $attachment) {
                                $resourceAttachment = DatabaseModel::select([
                                    'select'    => ['res_attachments.docserver_id', 'path_template', "replace(path,'#','/') as path", 'filename'],
                                    'table'     => ['res_attachments', 'docservers'],
                                    'left_join' => ['res_attachments.docserver_id = docservers.docserver_id'],
                                    'where'     => ['res_id = ?', "LOWER(format) = 'pdf'"],
                                    'data'      => [$attachment['res_id']]
                                ])[0];  // resource of the attachment of the exported document
                                if($resourceAttachment) {
                                    $tmpAttachmentFilePath = $resourceAttachment['path_template'] . $resourceAttachment['path'] . $resourceAttachment['filename'];
                                    $attachmentExtension = pathinfo($tmpAttachmentFilePath)['extension'];
                                    $attachmentName = self::generate_name($args['resources'][$i], $view, $attachmentSuffix, $attachmentNumber, $attachmentExtension, $globality);
                                    $attachmentFinalName = $path . '' . $attachmentName;
                                    if(copy($tmpAttachmentFilePath, $attachmentFinalName)) {
                                        $attachmentTime = strtotime(self::retrieve_creation_date($attachment['res_id'], $tableAttachment));
                                        touch($attachmentFinalName, $attachmentTime);
                                        if ($exportToZip) array_push($listOfFiles, $attachmentFinalName);
                                    }else {
                                        return ['errors' => ['[exportPESV2] [resId ' . $args['resources'][$i] . ']' . _ATTACHMENT_COPY_ERROR]];
                                    }
                                    $attachmentNumber = $attachmentNumber + 1;
                                }else {
                                    if (count($args['resources']) == 1) {
                                        return ['errors' => ['[exportPESV2] [resId ' . $args['resources'][$i] . ']' . _ERROR_WHILE_RETRIEVING_ATTACHMENT_FILE]];
                                    }
                                }
                            }
                        }
                        if (isset($status_after_action) && $status_after_action != '_NOSTATUS_') {
                            ResModel::update([
                                'set' => [
                                    'status' => $status_after_action
                                ],
                                'where' => ['res_id = ?'],
                                'data' => [$args['resources'][$i]]
                            ]);
                        }
                        $resIdList[$i] = $args['resources'][$i];
                    }else {
                        if (count($args['resources']) == 1)
                            return ['errors' => ['[exportPESV2] ' . $args['resources'][$i] . ']' . _COPY_ERROR]];
                    }
                } else {
                    if (count($args['resources']) == 1)
                        return ['errors' => ['[exportPESV2] ' . $args['resources'][$i] . ']' . _ERROR_WHILE_RETRIEVING_FILE]];
                }
            }
            if ($exportToZip == 'true') {
                self::PESV2Log('----- Export to zip option is enabled, compress all the PDF and delete them', $loggingEnabled);
                $zip = new ZipArchive;
                $zipFilename = $path . '/' . date($zipFilenameTimestamp) . '.zip';
                if ($zip->open($zipFilename, ZipArchive::CREATE)!==TRUE) {
                    return ['errors' => ['[exportPESV2] ' . _ERROR_CREATING_ZIP]];
                }
            ...
            self::PESV2Log('------ END of process PESV2', $loggingEnabled);
        } else {
            return ['errors' => ['[exportPESV2] ' . _CONFIG_FILE_ERROR]];
        }
        return true;
    }

    private static function convert_database_field($nameArray, $resId, $view){
        for($i = 0; $i < count($nameArray); $i++){
            $isCustom = $nameArray[$i]['isCustom'];
            $type = $nameArray[$i]['type'];
            $value = self::filter_filename($nameArray[$i]['value']);
            ...
            if($type == 'column'){
                ...
                $nameArray[$i]['value'] = self::filter_filename(trim($stmt));
            }
        }
        return $nameArray;
    }

    private static function convert_format($extensionArray, $resId, $view){
    ...
    private static function generate_name($resId, $view, $suffixAttachment = NULL, $attachmentNumber = '', $attachmentExtension = NULL, $structure){
        $tmp = $structure;
        $elements = $tmp[0]['ELEMENTS'];
        $extension = $tmp[0]['EXTENSION'];
        $separator = $tmp[0]['SEPARATOR'];

        $elements = self::convert_database_field($elements, $resId, $view);
        ...
        if(isset($attachmentExtension)){
            ...
            $extension = self::convert_format($extension, $resId, $view);
        }

        $string = self::convert_in_string($elements, $extension, $separator, $suffixAttachment, $attachmentNumber);

        return $string;
    }

    private static function convert_in_string($elements, $extension, $separator, $suffixAttachment, $attachmentNumber){
        $tmpArray = array();
    ...
    private static function filter_filename($name) {
        ...
        $sanitizeArr = [];
        $loadedXml = CoreConfigModel::getXmlLoaded(['path' => 'apps/maarch_entreprise/xml/file_export.xml']);
        foreach ($loadedXml -> SANITIZER as $settings) {
    ...
    private static function check_status($resId, $table, $status){
        $db = new \SrcCore\models\DatabaseModel();
    ...
    private static function retrieve_creation_date($resId, $table){
        $db = new \SrcCore\models\DatabaseModel();
    ...
    private static function retrieve_attachments($resIdMaster, $table, $authorizedAttachmentType){
        $db = new \SrcCore\models\DatabaseModel();
