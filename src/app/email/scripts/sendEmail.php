...
EmailScript::send(['customId' => $argv[1], 'emailId' => $argv[2], 'userId' => $argv[3], 'encryptKey' => $argv[4], 'options' => $options, 'receivedReceipt' => $argv[6], 'pjLinkChecked' => $argv[7]]); // EDISSYUM - NCH01 Ajout d'un accusé de lecture ajout receivedReceipt. AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères AJOUT de pjLinkChecked
...
class EmailScript
{
    public static function send(array $args)
    {
        ...
        $isSent = EmailController::sendEmail(['emailId' => $args['emailId'], 'userId' => $args['userId'], 'receivedReceipt' => $args['receivedReceipt'], 'pjLinkChecked' => $args['pjLinkChecked']]); // EDISSYUM - NCH01 Ajout d'un accusé de lecture ajout receivedReceipt. AMO01 - One_Line - Rajout d'une option -> mettre PJ dans mail via liens éphémères - Fonction check checkbox
