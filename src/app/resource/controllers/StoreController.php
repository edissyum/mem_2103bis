        ...
        foreach ($fields as $field) {
            ...
            if (empty($field['enabled']) || (!empty($field['mandatory']) && !isset($args[$field['identifier']]) && $defaultValue !== null)) {
                if (strpos($field['identifier'], 'indexingCustomField_') !== false) {
                    ...
                    $idCustom = $idCustom[1];
                    // EDISSYUM - NCH01 Fix afin d'éviter que la valeur par défaut d'un champs soit mise même après modification par un utilisateur
                    if ($args['customFields'][$idCustom] !== $defaultValue) {
                        continue;
                    }
                    // END EDISYSUM - NCH01
