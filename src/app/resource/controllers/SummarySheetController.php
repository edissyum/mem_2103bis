            ...
            if ($unit['unit'] == 'primaryInformations') {
                ...
                if (in_array('senders', $fieldsIdentifier)) {
                    $senders = ContactController::getFormattedContacts([
                        ...
                        'mode'  => 'sender',
                        'addNumber' => true // EDISSYUM - NCH01 Ajout du numéro de téléphone des contacts dans la fiche de liaison
                    ]);
        ...
        foreach ($units as $unit) {
            if ($unit['unit'] == 'notes') {
                $data['notes'] = NoteModel::get([
                    ...
                    'where'    => ['identifier in (?)'],
                    'data'     => [$tmpIds],
                    'orderBy' => ['id desc']]); // EDISSYUM - NCH01 Fix pour l'ordre des notes dans la fiche de liaison | remplacer order_by par orderBy et identifier par id
