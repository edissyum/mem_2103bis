...
use Entity\controllers\ListInstanceController; // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut
...
class ResController extends ResourceControlController
{
    ...
    public function update(Request $request, Response $response, array $args)
    {
        if (!Validator::intVal()->validate($args['resId'])) {
            ...
            // EDISSYUM - NCH01 Fix pour permettre la modification d'un courrier départ si le typist est égal à l'utilisateur connecté
            ...
            if ($resource['typist'] !== $GLOBALS['id'] && $resource['dest_user'] !== $GLOBALS['id'])
                // END EDISSYUM - NCH01 Fix pour permettre la modification d'un courrier départ si le typist est égal à l'utilisateur connecté
                return $response->withStatus(403)->withJson(['errors' => 'Service forbidden']);
        }
    ...
    public function updateStatus(Request $request, Response $response)
    {
        ...
        foreach ($identifiers as $id) {
            ...
            HistoryController::add([
                ...
                'eventType' => $eventType,  // NCH01 Add closing_date if status == 'END' --> Change 'UP' to $eventType
    ...
    public function getVersionFileContent(Request $request, Response $response, array $args)
    {
        ...
        //$fileContent = file_get_contents($pathToDocument); EDISSYUM - NCH01 Fix watermark (tag 21.03.28 Maarch)
        $fileContent = WatermarkController::watermarkResource(['resId' => $args['resId'], 'path' => $pathToDocument]); // EDISSYUM - NCH01 Fix watermark (tag 21.03.28 Maarch)
        if (empty($fileContent)) {
            $fileContent = file_get_contents($pathToDocument);
        }
        if ($fileContent === false) {
    ...
    public function getResourceFileInformation(Request $request, Response $response, array $args)
    {
        ...
        $resource = ResModel::getById([
            'resId'  => $args['resId'],
            'select' => ['format', 'fingerprint', 'filesize', 'fulltext_result', 'creation_date', 'filename', 'docserver_id', 'path', 'typist', 'dest_user', 'category_id'] // EDISSYUM - NCH01 Fix pour permettre la modification d'un courrier départ si le typist est égal à l'utilisateur connecté Add  dest_user et category_id
        ]);
        ...
        $resource['dest_user'] = $dest_user_infos['id'];
        // END EDISSYUM - NCH01
        ...
        if (!PrivilegeController::hasPrivilege(['privilegeId' => 'view_technical_infos', 'userId' => $GLOBALS['id']])) {
            $resource = [
                ...
                'dest_user'     => $dest_user_infos['id'], // EDISSYUM - NCH01 Fix pour permettre la modification d'un courrier départ si le typist est égal à l'utilisateur connecté
                'category_id'   => $resource['category_id'], // EDISSYUM - NCH01 Fix pour permettre la modification d'un courrier départ si le typist est égal à l'utilisateur connecté
                'typist'        => $resource['typist'], // EDISSYUM - NCH01 Fix pour permettre la modification d'un courrier départ si le typist est égal à l'utilisateur connecté
            ];
        }
    ...
    public function handleWebHook(Request $request, Response $response) {
        $loadedXml = CoreConfigModel::getXmlLoaded(['path' => 'apps/maarch_entreprise/xml/demarches_simplifiees.xml']);
        ...
        $auth = base64_encode($authArgs['maarchUserWsLogin'] . ':' . $authArgs['maarchUserWsPassword']);
        $body = $request->getParsedBody();
