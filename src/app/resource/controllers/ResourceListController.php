...
class ResourceListController
{
    ...
    public static function getSelectDataContacts(array $args)
    {
        $select = [
            'contacts.id as contact_id', 'lastname', 'firstname', 'company', 'department', 'function', 'email', 'phone',
            'address_number as addressNumber', 'address_street as addressStreet', 'address_additional1 as addressAdditional1',
            'address_additional2 as addressAdditional2', 'address_postcode as addressPostcode', 'address_town as addressTown',
            'address_country as addressCountry', 'creation_date', 'modification_date', 'custom_fields'
        ];

        foreach ($args['listDisplay'] as $value) {
            $value = (array)$value;
            if ($value['value'] == 'getCivility') {
                $select[] = 'contacts_civilities.label as "contacts_civilities.label"';
                $tableFunction[] = 'contacts_civilities';
                $leftJoinFunction[] = 'contacts.civility = contacts_civilities.id';
            } elseif ($value['value'] == 'getModificationDate') {
                $select[] = 'contacts.modification_date as "contacts.modification_date"';
            } elseif (strpos($value['value'], 'indexingCustomField_') !== false && !in_array('res_letterbox.custom_fields', $select)) {
                $select[] = 'contacts.custom_fields';
            }
        }
    ...
    public static function getFormattedResourcesContacts(array $args)
    {
        ValidatorModel::notEmpty($args, ['contacts', 'userId']);
        ValidatorModel::arrayType($args, ['contacts', 'listDisplay']);
        ...
        $mapping = [
            ...
            'getAddressPostCode' => 'addressPpostcode',
            'getCivility' => 'contacts_civilities.label',
        ...
        foreach ($contacts as $key => $contact) {
            $formattedResources[$key]['contact_id']       = $contact['contact_id'];
            $formattedResources[$key]['firstname']        = $contact['firstname'];
            $formattedResources[$key]['lastname']         = $contact['lastname'];
            $formattedResources[$key]['company']          = $contact['company'];
            $formattedResources[$key]['customFields']     = json_decode($contact['custom_fields']);
            ...
            if (isset($args['listDisplay'])) {
                ...
                foreach ($args['listDisplay'] as $value) {
                    ...
                    if ($mapping[$value['value']]) {
                        $value['displayValue'] = $contact[$mapping[$value['value']]];
                        $display[] = $value;
                    } elseif (strpos($value['value'], 'contactCustomField_') !== false) {
                        $customId = explode('_', $value['value'])[1];
                        $customValue = json_decode($contact['custom_fields'], true);
