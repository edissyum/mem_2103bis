...
class FolderPrintController
{
    ...
    public function generateFile(Request $request, Response $response)
    {
        ...
        foreach ($body['resources'] as $resource) {
            ...
            // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout && ($body['summarySheetMandatory'] || $body['summarySheet']) et mettre "!empty($unitsSummarySheet) || !empty($resource['summarySheet'])" entre parenthèse
            $withSummarySheet = (!empty($unitsSummarySheet) || !empty($resource['summarySheet'])) && ($body['summarySheetMandatory'] || $body['summarySheet']);
            ...
            if (!empty($resource['attachments'])) {
                if (is_array($resource['attachments'])) {
                    ...
                    $attachments = AttachmentModel::get([
                        ...
                        'where'   => ['res_id in (?)', 'status not in (?)', 'status <> ?', 'attachment_type <> ?'], // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout 'status <> ?', 'attachment_type <> ?'
                        'data'    => [$resource['attachments'], ['DEL', 'OBS'], 'SIGN', 'signed_response'], // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout 'SIGN', 'signed_response'
                    ...
                    $attachments = AttachmentModel::get([
                        'select'  => ['res_id', 'res_id_master', 'recipient_type', 'recipient_id', 'typist', 'status', 'attachment_type',
                                      'creation_date', 'identifier', 'title', 'format', 'docserver_id', 'origin'],
                        'where'   => ['res_id_master = ?', 'status not in (?)', 'status <> ?', 'attachment_type <> ?'], // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout 'status <> ?', 'attachment_type <> ?'
                        'data'    => [$resource['resId'], ['DEL', 'OBS'], 'SIGN', 'signed_response'], // EDISSYUM - NCH01 Amélioration de l'écran d'impression en masse | Ajout 'SIGN', 'signed_response'
                        'orderBy' => ['creation_date desc']
                    ]);
                }
            ...
            if (!empty($resource['attachments_signed'])) {
                $attachments = AttachmentModel::get([
                    'select'  => ['res_id', 'res_id_master', 'recipient_type', 'recipient_id', 'typist', 'status', 'attachment_type',
                        'creation_date', 'identifier', 'title', 'format', 'docserver_id', 'origin'],
                    'where'   => ['res_id_master = ?', 'status not in (?)', "(status = ? AND attachment_type in ('signed_response', 'response_project'))"],
                    'data'    => [$resource['resId'], ['DEL', 'OBS'], 'SIGN'],
                    'orderBy' => ['creation_date desc']
                ]);
            ...
            if (!empty($documentPaths)) {
                ...
                $filePathOnTmp = str_replace(' ', '_', $filePathOnTmp); // EDISSYUM - NCH01 Sanitize filename pour dossier d'impression
                $filePathOnTmp = str_replace("'", '', $filePathOnTmp); // EDISSYUM - NCH01 Sanitize filename pour dossier d'impression
                $filePathOnTmp = str_replace('"', '', $filePathOnTmp); // EDISSYUM - NCH01 Sanitize filename pour dossier d'impression
