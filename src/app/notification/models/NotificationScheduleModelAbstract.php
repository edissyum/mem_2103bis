...
abstract class NotificationScheduleModelAbstract
{
    public static function saveCrontab(array $aArgs = [])
    {
        ...
        $uniqID = uniqid(); // EDISSYUM - NCH01 Améliorations fichier temporaire lors de la planification de notifications
        $output = preg_replace("!\n+$!", "\n", $output);
        file_put_contents('/tmp/crontab_' . $uniqID . '.plain', print_r($file, true)); // EDISSYUM - NCH01 Améliorations fichier temporaire lors de la planification de notifications
        file_put_contents('/tmp/crontab_' . $uniqID . '.txt', $output); // EDISSYUM - NCH01 Améliorations fichier temporaire lors de la planification de notifications

        exec('crontab /tmp/crontab_' . $uniqID . '.txt'); // EDISSYUM - NCH01 Améliorations fichier temporaire lors de la planification de notifications

        unlink("/tmp/crontab_" . $uniqID . ".txt"); // EDISSYUM - NCH01 Améliorations fichier temporaire lors de la planification de notifications
        unlink("/tmp/crontab_" . $uniqID . ".plain"); // EDISSYUM - NCH01 Améliorations fichier temporaire lors de la planification de notifications
