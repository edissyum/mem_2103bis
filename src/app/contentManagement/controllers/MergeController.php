...
class MergeController
{
    ...
    private static function getDataForMerge(array $args)
    {
        ...
        // EDISSYUM - NCH01 Rajout des variables de fusions pour remonter jusqu'au type d'entité choisie
        $entityTypes = EntityModel::getTypes();
        foreach ($entityTypes as $key => $value) {
            if (strtolower($value['fusion']) == 'true') {
                $dataToBeMerge['destinationEntityType_' . $value['id']] = [];
                if (!empty($destination)) {
                    if ($destination['entity_type'] == $value['id']) {
                        $dataToBeMerge['destinationEntityType_' . $value['id']] = $destination;
                    } else {
                        $dataToBeMerge['destinationEntityType_' . $value['id']] = self::retrieveDestinationByEntityType($destination, $value['id']);
                    }
                }
            }
        }
    ...
    // EDISSYUM - NCH01 Rajout des variables de fusions pour remonter jusqu'au type d'entité choisie
    private static function retrieveDestinationByEntityType($childDestination, $entityType) {
        if (isset($childDestination['parent_entity_id']) && !empty($childDestination['parent_entity_id'])) {
            $parentDestination = EntityModel::getByEntityId(['entityId' => $childDestination['parent_entity_id'], 'select' => ['*']]);
            $parentDestination['path'] = EntityModel::getEntityPathByEntityId(['entityId' => $parentDestination['entity_id'], 'path' => '']);
            if ($parentDestination['entity_type'] == $entityType) {
                return $parentDestination;
            } else {
                return self::retrieveDestinationByEntityType($parentDestination, $entityType);
            }
        }
    }
    // END EDISSYUM - NCH01
...
//        $generator->writeBarcodeFile($barcodeFile);
        ...
        if (file_exists($qrcodeFile)) unlink($qrcodeFile); // EDISSYUM - NCH01 Fix pour éviter de supprimer un document qui n'existe pas | Ajout du if
        if (file_exists($barcodeFile)) unlink($barcodeFile); // EDISSYUM - NCH01 Fix pour éviter de supprimer un document qui n'existe pas | Ajout du if
