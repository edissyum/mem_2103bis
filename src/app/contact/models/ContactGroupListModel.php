...
class ContactGroupListModel
{
    ...
    public static function getWithCorrespondents(array $args = [])
    {
        ...
        $lists = DatabaseModel::select([
            'select'    => empty($args['select']) ? ['*'] : $args['select'],
            'table'     => ['contacts_groups_lists', 'contacts_groups', 'contacts', 'entities', 'users'], // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre | ajout contacts_groups
            'left_join' => [
                "contacts_groups_lists.correspondent_id = contacts_groups.id AND contacts_groups_lists.correspondent_type = 'contactGroup'", // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre
