...
class ContactController
{
    ...
    // EDISSYUM - NCH01 Récupération des documents liés à un contact pour Open-Capture
    public function getResourcesByContactId(Request $request, Response $response, array $args) {
        if (!Validator::IntVal()->notEmpty()->validate($args['id'])) {
            return $response->withStatus(400)->withJson(['errors' => 'Argument id is empty or not a string']);
        }

        $where = ['item_id = ?', 'type = ?', ];
        $select = ['r.res_id, r.subject', 'alt_identifier'];
        $queryParams = $request->getQueryParams();
        if (!empty($queryParams['custom_fields'])) {
            array_push($where, "custom_fields ->> '" . $queryParams['custom_fields'] . "' != ''");
            array_push($select, "custom_fields ->> '" . $queryParams['custom_fields'] . "' as data");
        }

        if (!empty($queryParams['clause'])) {
            array_push($where, $queryParams['clause']);
        }

        $resources = DatabaseModel::select([
            'select'    => $select,
            'table'     => ['resource_contacts rc', 'res_letterbox r'],
            'left_join' => ['rc.res_id = r.res_id'],
            'where'     => $where,
            'data'      => [$args['id'], 'contact'],
        ]);

        if (count($resources) == 0) {
            return $response->withStatus(204)->withJson(['errors' => 'No document']);
        }
    ...
    {
        // EDISSYUM - NCH01 Rajout de la possibilité de visualiser les contacts sans droits d'administration | dans le if, rajout du && ......
        if (!PrivilegeController::hasPrivilege(['privilegeId' => 'admin_contacts', 'userId' => $GLOBALS['id']])
            && !PrivilegeController::hasPrivilege(['privilegeId' => 'view_contacts', 'userId' => $GLOBALS['id']])) {
        ...
        if (!empty($queryParams['search'])) {
            $fields = ['firstname', 'lastname', 'company', 'address_number', 'address_street', 'address_additional1', 'address_additional2', 'address_postcode', 'address_town', 'address_country', 'email']; // EDISSYUM - NCH01 Module formulaire SVE
        ...
        // EDISSYUM - NCH01 Récupération des documents liés à un contact pour Open-Capture
        if ($queryParams['where'] && !$requestData['where']) {
            $requestData['where'] = [$queryParams['where']];
        }
        // END EDISSYUM - NCH01

        $contacts = ContactModel::get([
            'select'    => [
                'id', 'firstname', 'lastname', 'company', 'address_number as "addressNumber"', 'address_street as "addressStreet"', 'custom_fields', // EDISSYUM - NCH01 Rajout de la confidentialité des contacts ajout de custom_fields
                'address_additional1 as "addressAdditional1"', 'address_additional2 as "addressAdditional2"', 'address_postcode as "addressPostcode"',
                'address_town as "addressTown"', 'address_country as "addressCountry"', 'enabled', 'sector', 'count(1) OVER()', 'email' // EDISSYUM -NCH01 Module formulaire SVE
            ],
        ...
        foreach ($contacts as $key => $contact) {
            ...
            $contacts[$key]['customFields'] = !empty($contact['custom_fields']) ? json_decode($contact['custom_fields'], true) : null; // EDISSYUM - NCH01 Rajout de la confidentialité des contacts
            unset($contacts[$key]['custom_fields']); // EDISSYUM - NCH01 Rajout de la confidentialité des contacts
    ...
    public function create(Request $request, Response $response)
    {
        ...
        if (!empty($body['email']) && $currentUser['mode'] == 'rest') {
            ...
            if (!empty($contact[0]['id'])) {
                // EDISSYUM - NCH01 Fix création de doublon de contact
                // return $response->withJson(['id' => $contact[0]['id']]);
                if ($currentUser['mode'] !== 'rest') {
                    return $response->withStatus(400)->withJson(['errors' => "L'adresse e-mail existe déjà"]);
                }
                return $response->withJson(['id' => $contact[0]['id']]);
                // END EDISSYUM - NCH01 Fix création de doublon de contact
            }
        }
        ...
        foreach ($data['contactsParameters'] as $contactParameter) {
            ...
            ContactParameterModel::update([
                'set'   => [
                    ...
                    'filtrable' => empty($contactParameter['filtrable']) ? 'false' : 'true' // EDISSYUM - NCH01 Fenetre de recherche de contacts
                ],
                'where' => ['id = ?'],
    ...
    public function getAvailableDepartments(Request $request, Response $response)
    {
        ...
        if (empty($departments)) {
            return $response->withJson(['departments' => [], 'default' => null]); // EDISSYUM - NCH01 Fix pour éviter une erreur si la BAN n'est pas accessible
        }
    ...
    public function getDuplicatedContacts(Request $request, Response $response)
    {
        ...
        foreach ($queryParams['criteria'] as $criterion) {
            if (strpos($criterion, 'contactCustomField_') !== false) {
                ...
                // EDISSYUM - OBR01 Fix de type d'argument pour la fonction unaccent
                $criteria[] = "replace(lower(unaccent(" . $allowedFields[$criterion] . "::TEXT) ), ' ', '')";
                // END EDISSYUM - OBR01
            }
        }
        ...
        // EDISSYUM - NCH01 Ajout d'un paramètre pour gérer le nombre max de contacts afficher dans l'écran de dédoublonnage
        $limit = 500;
        $limitParameter = ParameterModel::getById(['id' => 'ContactsDuplicateMaxItems']);
        if (count($limitParameter) > 0) {
            $limit = $limitParameter['param_value_int'];
        }
        // END EDISSYUM - NCH01
        ...
        $duplicates = DatabaseModel::select([
            ...
            'order_by' => $order,
            'limit'    => $limit // EDISSYUM - NCH01 Ajout d'un paramètre pour gérer le nombre max de contacts afficher dans l'écran de dédoublonnage | Remplacer 500 par $limit
        ]);
    ...
    {
        if (!PrivilegeController::hasPrivilege(['privilegeId' => 'admin_contacts', 'userId' => $GLOBALS['id']]) &&
            !PrivilegeController::hasPrivilege(['privilegeId' => 'admin_search_contacts', 'userId' => $GLOBALS['id']])) { // EDISSYUM - NCH01 Fenetre de recherche de contacts | ajout du privileges admin_search_contacts
            return $response->withStatus(403)->withJson(['errors' => 'Service forbidden']);
        }
        ...
        $where = ['1 = ?'];
        $data = ['1'];
        if (isset($body['selectedContacts']) && !empty($body['selectedContacts'])) {
            $where = ['id IN (' . implode(',', $body['selectedContacts']) . ')'];
            $data = [];
        }
        // END EDISSYUM - NCH01

        $contacts = ContactModel::get(['select' => $fields, 'where' => $where, 'data' => $data]); // EDISSYUM - NCH01 Fenetre de recherche de contacts ajout de $where et $data
    ...
    public static function getFormattedContacts(array $args)
    {
        ...
        foreach ($resourceContacts as $resourceContact) {
            ...
            if ($resourceContact['type'] == 'contact') {
                ...
                // EDISSYUM - NCH01 Ajout du numéro de téléphone des contacts dans la fiche de liaison
                if (isset($args['addNumber']) and $args['addNumber']) {
                    $contact .= "<br>" . $contactRaw['phone'];
                }
                // END EDISSYUM - NCH01
            } elseif ($resourceContact['type'] == 'user') {
    ...
    // EDISSYUM - NCH01 opencaptureformem
