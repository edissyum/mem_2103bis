...
use Parameter\models\ParameterModel; // EDISSYUM - NCH01 Ajout d'un paramètre pour gérer le nombre max lors d'un publipostage
...
class ContactGroupController
{
    ...
    public function getByLabel(Request $request, Response $response, array $args)
    {
        $body = $request->getParsedBody();
        if (!Validator::stringType()->notEmpty()->validate($body['label'] ?? null)) {
            return $response->withStatus(400)->withJson(['errors' => 'Body label is empty or not a string']);
        }
        $contactsGroups = ContactGroupModel::get(['where' => ['label = ?'], 'data' => [$body['label']]]);
        return $response->withJson(['contactsGroup' => $contactsGroups]);
    }
    // END EDISSYUM - NCH01
    ...
    public function getCorrespondentsById(Request $request, Response $response, array $args)
    {
        ...
        // EDISSYUM - NCH01 Ajout d'un paramètre pour gérer le nombre max lors d'un publipostage
        $limit = 25;
        $limitParameter = ParameterModel::getById(['id' => 'PublipostageMaxItems']);
        if (count($limitParameter) > 0) {
            $limit = $limitParameter['param_value_int'];
        }
        // END EDISSYUM - NCH01

        $queryParams['limit'] = (empty($queryParams['limit']) || !is_numeric($queryParams['limit']) ? $limit : (int)$queryParams['limit']); // EDISSYUM - NCH01 Ajout d'un paramètre pour gérer le nombre max lors d'un publipostage - Changer le 25 en $limit
        ...
        if (!empty($queryParams['search'])) {
            // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre | supprimer contacts.sector | Ajouter contacts_groups.label
            $fields = [
            ...
            $requestData = AutoCompleteController::getDataForRequest([
                ...
                'where'         => $where,
                'data'          => $data,
                'fieldsNumber'  => 16, // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre | Passer à 16
            ]);
        ...
        foreach ($rawCorrespondents as $correspondent) {
            ...
            elseif ($correspondent['correspondent_type'] == 'contactGroup') {
                $contactGroup = ContactGroupModel::getById([
                    'select'    => ['id', 'label'],
                    'id'        => $correspondent['correspondent_id']
                ]);
                $correspondents[] = [
                    'id'    => $correspondent['correspondent_id'],
                    'type'  => $correspondent['correspondent_type'],
                    'name'  => $contactGroup['label'],
                    'address' => 'Groupement de contacts'
                ];
            }
            // END EDISSYUM - NCH01
        }
    ...
    public function addCorrespondents(Request $request, Response $response, array $args)
    {
        ...
        foreach ($body['correspondents'] as $correspondent) {
            ...
            // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre | Ajout de contactGroup dans le in_array
            if (!in_array($correspondent['id'], $correspondents[$correspondent['type']]) && in_array($correspondent['type'], ['contact', 'user', 'entity', 'contactGroup'])) {
                ContactGroupListModel::create([
