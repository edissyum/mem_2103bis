...
class FolderController
{
    ...
    public function getResourcesById(Request $request, Response $response, array $args)
    {
        ...
        $adminSearchFolders = ConfigurationModel::getByPrivilege(['privilege' => 'admin_search_folders', 'select' => ['value']]);

        if (empty($adminSearchFolders)) {
            return $response->withStatus(400)->withJson(['errors' => 'No admin_search_folders configuration found', 'lang' => 'noAdminSearchConfiguration']);
        }
        // END EDISSYUM - EME01
        ...
        if (!empty($foldersResources)) {
            ...
            // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers
            $configuration = json_decode($adminSearchFolders['value'], true);
            ...
            if (!empty($resIds)) {
                $attachments = AttachmentModel::get([
                    ...
                    'data'      => [$resIds, ['DEL', 'OBS'], 'TMP', $GLOBALS['id'], 'TMP', AttachmentTypeController::HIDDEN_ATTACHMENT_TYPES],
                    'groupBy'   => ['res_id_master']
                ]);


// EDISSYUM - EME01  Ajout d'une fenêtre pour administrer la recherche des dossiers | Commenter les variables $select && $tableFunction && $leftJoinFunction
//                $select = [
...
// END EDISSYUM - EME01
                ...
                $selectData = ResourceListController::getSelectData(['listDisplay' => $listDisplay]); // EDISSYUM - EME01  Ajout d'une fenêtre pour administrer la recherche des dossiers

                $resources = ResourceListModel::getOnResource([
                    'select'    => $selectData['select'], // EDISSYUM - EME01  Ajout d'une fenêtre pour administrer la recherche des dossiers | remplacer $select par $selectData['select']
                    'table'     => $selectData['tableFunction'], // EDISSYUM - EME01  Ajout d'une fenêtre pour administrer la recherche des dossiers | remplacer $tableFunction par $selectData['tableFunction']
                    'leftJoin'  => $selectData['leftJoinFunction'], // EDISSYUM - EME01  Ajout d'une fenêtre pour administrer la recherche des dossiers | remplacer $leftJoinFunction par $selectData['leftJoinFunction']
                ...
                $formattedResources = ResourceListController::getFormattedResources([
                    ...
                    'checkLocked'   => false,
                    'listDisplay'   => $listDisplay,  // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers | remplacer ['folders'] par $listDisplay
                    'trackedMails'  => $followedResources // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers
                ]);
        ...
        return $response->withJson([
            ...
            'defaultTab'        => $configuration['listEvent']['defaultTab'], // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers | ajouter  'defaultTab'        => $configuration['listEvent']['defaultTab'],
            'templateColumns'   => $configuration['listDisplay']['templateColumns'], // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers | ajouter 'templateColumns'   => $configuration['listDisplay']['templateColumns']
            'displayFolder'     => $displayFolder  // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers  | ajouter 'displayFolder'     => $displayFolder
        ]);
    }
    ...
    public static function getScopeFolders(array $args)
    {
        ...
        // EDISSYUM - EME01 Afficher les dossiers dans l'ordre de création par le script de l'autofoldering
        $orderBy = ['level', 'label desc'];

        $autofolderingConfig = CoreConfigModel::getJsonLoaded(['path' => 'apps/maarch_entreprise/xml/autofoldering.json']);
        if (!empty($autofolderingConfig['enabled'])) {
            $orderBy = ['level', 'id desc', 'label desc'];
        }
        // END EDISSYUM - EME01
        ...
        $folders = FolderModel::getWithEntities([
            ...
            'where'     => $where,
            'data'      => $data,
            'orderBy'   => $orderBy // EDISSYUM - EME01 Afficher les dossiers dans l'ordre de création par le script de l'autofoldering
        ]);
    ...
    {
        $configuration = ConfigurationModel::getByPrivilege(['privilege' => 'admin_search_folders']);
