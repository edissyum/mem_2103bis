...
class UserController
{
    ...
    public function get(Request $request, Response $response)
    {
        ...
        $userQuotaFiltered = ParameterModel::getById(['id' => 'user_quota_filtered', 'select' => ['param_value_string']]); // EDISSYUM - NCH01 Ajout d'un filtre sur le userquota
        if (!empty($userQuota['param_value_int'])) {
            // EDISSYUM - NCH01 Ajout d'un filtre sur le userquota
            $where = ['status = ?', 'mode != ?'];
            if ($userQuotaFiltered['param_value_string']) {
                array_push($where, 'user_id NOT IN (' . $userQuotaFiltered['param_value_string'] . ')');
            }
            // END EDISSYUM - NCH01
            $activeUser = UserModel::get(['select' => ['count(1)'], 'where' => $where, 'data' => ['OK', 'root_invisible']]); // EDISSYUM - NCH01 Ajout d'un filtre sur le userquota
            $inactiveUser = UserModel::get(['select' => ['count(1)'], 'where' => ["status IN ('SPD', 'INACT')", 'mode != ?'], 'data' => ['root_invisible']]); // EDISSYUM - NCH01 Ajout d'un filtre sur le userquota
    ...
    {
        $user = UserModel::getById(['id' => $args['id'], 'select' => ['id', 'firstname', 'lastname', 'status', 'mail', 'phone', 'external_id', 'preferences']]); // EDISSYUM -NCH01 Amélioration des webservices utilisateurs | Ajout external_id et preferences
        $user['external_id'] = json_decode($user['external_id']); // EDISSYUM -NCH01 Amélioration des webservices utilisateurs
        $user['preferences'] = json_decode($user['preferences']); // EDISSYUM -NCH01 Amélioration des webservices utilisateurs
        if (empty($user)) {
    ...
    public function update(Request $request, Response $response, array $aArgs)
    {
        ...
        $user = UserModel::getById(['id' => $aArgs['id'], 'select' => ['status', 'mode', 'external_id', 'preferences']]); // EDISSYUM - NCH01 Amélioration des webservices utilisateurs | Ajout d'external_id et preferences
        ...
        // EDISSYUM - NCH01 Amélioration des webservices utilisateurs
        if (!empty($body['external_id'])) {
            $externalId = json_decode($user['external_id'], true);
            foreach($body['external_id'] as $key => $value) {
                if(!is_null($value)) {
        ...
        if (!empty($body['preferences'])) {
            $preferences = json_decode($user['preferences'], true);
            foreach($body['preferences'] as $key => $value) {
                if(!is_null($value)) {
    ...
    public function suspend(Request $request, Response $response, array $args)
    {
        ...
        $user = UserModel::getById(['id' => $args['id'], 'select' => ['firstname', 'lastname', 'user_id', 'mode', 'status']]); // EDISSYUM - NCH01 Fix pour éviter le passage en SPD des utilisateurs INACT lors de la synchro LDAP | rajout de status
        ...
        // EDISSYUM - NCH01 Fix pour éviter la suppression d'un utilisateur LDAP si présent dans le circuit de visa
        $body = $request->getParsedBody();
        if (!isset($body['actionMode'])) { // Si l'appel à ce WebService est fait depuis l'interface, ne pas effectuer ce test
            $listInstances = ListInstanceModel::getWhenOpenMailsByUserId(['select' => [1], 'userId' => $args['id'], 'itemMode' => 'cc']);
            if (!empty($listInstances)) {
                return $response->withStatus(403)->withJson(['errors' => 'User is still present in listInstances']);
            }
            $listTemplates = ListTemplateModel::getWithItems([
                'select'    => [1],
                'where'     => ['item_id = ?', 'type = ?', "item_mode IN ('visa', 'sign')", 'item_type = ?', 'entity_id is not null'],
                'data'      => [$args['id'], 'visaCircuit', 'user']
            ]);
            if (!empty($listTemplates)) {
                return $response->withStatus(403)->withJson(['errors' => 'User is still present in listTemplates']);
            }
        ...
        $loginMethod = CoreConfigModel::getLoggingMethod(); // EDISSYUM - NCH01 Fix pour éviter le passage en SPD des utilisateurs INACT lors de la synchro LDAP

        UserModel::update([
            'set'   => [
                'status'   => $loginMethod['id'] == 'ldap' && $user['status'] == 'INACT' ? 'INACT' : 'SPD' // EDISSYUM - NCH01 Fix pour éviter le passage en SPD des utilisateurs INACT lors de la synchro LDAP
            ],
            'where' => ['id = ?'],
            'data'  => [$args['id']]
        ]);
    ...
    // EDISSYUM - NCH01 Rajout d'une route pour récupérer les informations d'un utilisateur
    public function getUserByUserId(Request $request, Response $response, array $aArgs)
    {
        if (!PrivilegeController::hasPrivilege(['privilegeId' => 'admin_users', 'userId' => $GLOBALS['id']])) {
            return $response->withStatus(403)->withJson(['errors' => 'Service forbidden']);
        }

        $user = UserModel::getByLowerLogin(['login' => $aArgs['userId'], 'select' => ['*']]);

        if (empty($user)) {
            return $response->withJson(['user' => null]);
        }
    ...
    public function updateExternalIdByUserId(Request $request, Response $response, array $aArgs)
    {
        $user = UserModel::getByLowerLogin(['login' => $aArgs['userId'], 'select' => ['id', 'user_id', 'external_id']]);
        if (empty($user)) {
            return $response->withStatus(400)->withJson(['errors' => 'User ' . $aArgs['userId'] . ' does not exist']);
        }
        $error = $this->hasUsersRights(['id' => $user['id']]);
        if (!empty($error['error'])) {
            return $response->withStatus($error['status'])->withJson(['errors' => $error['error']]);
        }

        $data = $request->getParams();
        $check = Validator::arrayType()->notEmpty()->validate($data['external_id']);
        if (!$check) {
            return $response->withStatus(400)->withJson(['errors' => 'Bad Request']);
        }

        $externalId = json_decode($user['external_id'], true);
        foreach($data['external_id'] as $key => $value) {
            if(!is_null($value)) {
                $externalId[$key] = $value;
            }
        }
        ...
        return $response->withJson(['user' => UserModel::getById(['id' => $user['id'], 'select' => ['external_id']])]);
    }
    // END EDISSYUM - PYB01
}
