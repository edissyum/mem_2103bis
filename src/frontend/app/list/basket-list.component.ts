...
export class BasketListComponent implements OnInit, OnDestroy {
    ...
    togglePanel(mode: string, row: any) {
        ...
        if (previousRes.resId === row.resId && this.sidenavRight.opened && this.currentMode === mode) {
            ...
            row['basketId'] = this.currentBasketInfo.basketId; // EDISSYUM - NCH01 Fix pour afficher les templates de notes en fonction de l'entitié du user et non de la destination du courrier
    ...
    viewThumbnail(row: any) {
        if (row.hasDocument && !this.appService.getViewMode()) { // EDISSYUM - NCH01 Fix pour IOS
    ...
    closeThumbnail() {
        $('#viewThumbnail').hide();
        $('#listContent').css({ 'overflow': 'scroll' });
    }
