...
export class SelectIndexingModelComponent implements OnInit {
    ...
    getIndexingModelList() {
        // EDISSYUM - NCH01 Afficher les modèles d'enregistrement désactivés dans la fiche détaillé
        let url = '';
        if (window.location.href.includes('resources')) {
            url = '../rest/indexingModels?showDisabled=true';
        }else{
            url = '../rest/indexingModels';
        }
