...
export class IndexingFormComponent implements OnInit {
    ...
    @Input() canChangeModel: boolean = true; // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut
            ...
            if (element.type === 'date' && !this.functions.empty(this.arrFormControl[element.identifier].value)) {
                if (element.today === true) {
                    ...
                    if (element.identifier === 'processLimitDate') {
                        ...
                        if (Object.prototype.toString.call(this.arrFormControl[element.identifier].value) !== '[object Date]') {
                            this.arrFormControl[element.identifier].value = new Date(this.arrFormControl[element.identifier].value);
                        }
                        // END EDISSYUM - NCH01
    ...
    saveData(fromProcess= false) { // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut - ajout de fromProcess = false
        return new Promise((resolve, reject) => {
            if (this.isValidForm()) {
                this.mustFixErrors = false;
                const formatdatas = this.formatDatas(this.getDatas());

                this.http.put(`../rest/resources/${this.resId}?fromProcess=` + fromProcess, formatdatas).pipe( // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut - ajout de fromProcess
                    tap(() => {
                ...
                tap((data: any) => {
                    ...
                    if (this.adminMode) {
                        ...
                        if (elem.default_value === '#myPrimaryEntity') {
                            ...
                            this.selfDest = false;
                            const defaultVal = data.entities.filter((entity: any) => entity.id === elem.default_value); // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut - Suppression de entity.enabled = true &&
                ...
                tap(async (data: any) => {
                    ...
                    await Promise.all(this.fieldCategories.map(async (element: any) => {
                        ...
                        await Promise.all(this['indexingModels_' + element].map(async (elem: any) => {
                            ...
                            if (Object.keys(data).indexOf(elem.identifier) > -1 || customId !== undefined) {
                                ...
                                if (elem.type === 'date') {
                                    if (!this.isPrivate && this.functions.empty(fieldValue)) {
                                        ...
                                        // EDISSYUM - NCH01 Fix pour IOS
                                        // fieldValue = new Date(fieldValue);
                                        fieldValue = new Date(fieldValue.replace(' ', 'T'));
                                        // END EDISSYUM - NCH01
                                    }
                                }
                            ...
                            // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut
                            if (this.canChangeModel && elem.default_value && elem.default_value.length !== 0) {
                                this.arrFormControl[elem.identifier].setValue(elem.default_value);
                            }
                            //  END EDISSYUM - NCH01
