...
export class SearchResultListComponent implements OnInit, OnDestroy {
    ...
    displayFolder: boolean = false;  // EDISSYUM - EME01 Fix sur l'affichage du nom du dossier (emplacement fixe) d'un courrier
        ...
        merge(this.sort.sortChange, this.paginator.page, this.filtersChange)
            .pipe(
                ...
                map((data: any) => {
                    ...
                    this.displayFolder = data.displayFolder; // EDISSYUM - EME01 Fix sur l'affichage du nom du dossier (emplacement fixe) d'un courrier
    ...
    processPostData(data: any) {
        data.resources.forEach((element: any) => {
            if (Object.keys(this.criteria).filter((crit) => crit.indexOf('role_') > -1).length > 0 && Object.keys(this.criteria).filter((crit) => crit.indexOf('role_typist') == -1).length > 0) { // EDISSYUM - NCH01 Ajout du typist dans la recherche (enlever le rond jaune autour du logo de la liste de diffusion)
