...
} from "@appRoot/search/criteria-tool/search-template/search-template-modal.component";
...
} from "@appRoot/administration/registered-mail/issuing-site/indexing/issuing-site-input.component";
import { SortPipe } from "@plugins/sorting.pipe";

@Component({
    selector: 'app-contact-criteria-tool',
    templateUrl: 'contacts-criteria-tool.component.html',
    styleUrls: ['contacts-criteria-tool.component.scss', '../../../indexation/indexing-form/indexing-form.component.scss'],
    providers: [DatePipe, SortPipe]
})
export class ContactsCriteriaToolComponent implements OnInit {

    @Input() searchTermContact: string = 'Foo';
    ...
    searchTermContactControl = new FormControl();
    searchCriteria = new FormControl();
    contactsCustomFields = {};
    civilities = {};
    correspondentsGroups = {};
    infoFields: any = [
        {
            id: 1,
    ...
    constructor(
        ...
        _activatedRoute.queryParams.subscribe(
            params => {
                this.searchTermContact = params.value;
            }
        );
    }

    async ngOnInit(): Promise<void> {
        this.searchTermContactControl.setValue(this.searchTermContact);
        const civilities = await this.getCivilities();
        // const contactsGroups = this.getContactsGroups();
        this.criteria = [
            {
            ...
            {
                identifier: 'contactsGroups',
                label: this.translate.instant('lang.contact_contactGroup'),
                icon: 'fa-users',
                type: 'select',
                enabled: true,
                values: []
            ...
            {
                identifier: 'addressadditional2',
                label: 'Complément d\'acheminement',
                icon: 'fa-map-marker-alt',
                mandatory: true,
                system: true,
                type: 'string',
        ...
        const contactsCustomFields = await this.getContactsCustomFields();
        this.criteria = this.criteria.concat(contactsCustomFields);
        ...
        setTimeout(() => {
            this.loading = false;
            this.searchTermContactControl.valueChanges
                .pipe(
                    startWith(''),
                    map(value => {
                        if (typeof value === 'string' && !this.functions.empty(value)) {
                            this.searchTermContact = value;
                        }
                    })
                ).subscribe();
        }, 500);
    ...
    getCivilities() {
        return new Promise((resolve, reject) => {
            this.http.get('../rest/civilities').pipe(
                ...
                finalize(() => resolve(this.civilities)),
                catchError((err: any) => {
    ...
    getContactsGroups() {
        return new Promise((resolve, reject) => {
            this.http.get('../rest/contactsGroups').pipe(
                ...
                finalize(() => {
                    resolve(this.correspondentsGroups);
                    this.loading = false;
    ...
    async getContactsCustomFields() {
        return new Promise((resolve, reject) => {
            this.http.get('../rest/contactsCustomFields').pipe(
                tap((data: any) => {
                    this.contactsCustomFields = data.customFields.map((info: any) => {
                        info.identifier = 'contactCustomField_' + info.id;
                ...
                finalize(() => resolve(this.contactsCustomFields)),
                catchError((err: any) => {
                    this.notify.handleSoftErrors(err);
                    return of(false);
                })
    ...
    async addCriteria(criteria: any, openPanel: boolean = true) {
        ...
        if (criteria.identifier === 'contactsGroups' && Object.keys(this.correspondentsGroups).length === 0) {
            this.loading = true;
            this.isLoadingResult = true;
            criteria.values = await this.getContactsGroups();
        }
        if (this.adminMode) {
    ...
    getCurrentCriteriaValues() {
        const objCriteria = {};
        if (!this.functions.empty(this.searchTermContactControl.value)) {
            objCriteria['meta'] = {
                values: this.searchTermContactControl.value
            };
        }
    ...
    getFormatLabel(identifier: string, value: any) {
        if (this.criteria.filter((field: any) => field.identifier === identifier).length > 0) {
    ...
    resetCriteria(criteriaId: string, value: any, refresh: boolean = true) {
        if (criteriaId !== 'meta') {
            ...
            this.searchTermContactControl.setValue('');
        }
        if (refresh) {
            this.getCurrentCriteriaValues();
        }
    ...
    set_meta_field(value: any) {
        this.searchTermContactControl.setValue(value);
    }
    ...
    getSearchTemplates() {
        this.http.get('../rest/searchContactsTemplates').pipe(
            tap((data: any) => {
    ...
    saveSearchTemplate() {
        ...
        query.push({ 'identifier': 'meta', 'values': this.searchTermContactControl.value });
    ...
    selectSearchTemplate(searchTemplate: any, openPanel: boolean = true) {
        ...
        if (index > -1) {
            this.searchTermContactControl.setValue(searchTemplate.query[index].values);
        }
    }
