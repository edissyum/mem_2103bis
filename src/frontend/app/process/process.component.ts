...
export class ProcessComponent implements OnInit, OnDestroy {
    ...
    changedIndexingFormModel: boolean = false; // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut
    ...
    async initDetailPage(params: any) {
        this._activatedRoute.queryParamMap.subscribe((paramMap: ParamMap) => {
            ...
            // this.isFromSearch = !this.functions.empty(paramMap.get('fromSearch')); // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers | Commenter cette ligne
            // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers
            if (!this.functions.empty(paramMap.get('fromSearch'))) {
                if (paramMap.get('fromSearch') === 'true' ) {
    ...
    loadResource(redirectDefautlTool: boolean = true) {
        // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut - remove ?light=true dans l'url du .get()
    ...
    setEditDataPrivilege() {
        if (this.detailMode) {
            ...
            // EDISSYUM - EME01 Ajout d'une fenêtre pour administrer la recherche des dossiers
                ...
                this.http.get('../rest/searchFolders').pipe(
                    tap((myData: any) => {
                        if (myData.configuration.listEvent.defaultTab == null) {
                            this.currentTool = 'dashboard';
                        } else {
    ...
    triggerProcessAction() {
        // EDISSYUM - NCH01 Fix pour éviter l'activation d'une action si toutes les infos ne sont pas renseignées - ADD TIMEOUT
        setTimeout(() => {
            if (this.autoAction) {
                this.processAction();
            ...
            if (this.isToolModified()) {
                ...
                dialogRef.afterClosed().pipe(
                    tap((data: string) => {
                        ...
                        } else { // EDISSYUM - NCH01 Fix pour éviter l'activation d'une action après le refus d'enregistrer depuis les courriers à qualifier
                            return of(false);
                        }
                        // END EDISSYUM - NCH01
                    }),
    ...
    isToolModified() {
        // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut - ajout de this.currentTool === 'info' && this.changedIndexingFormModel dans le if
        if (this.currentTool === 'info' && this.changedIndexingFormModel || this.currentTool === 'info' && this.indexingForm !== undefined && this.indexingForm.isResourceModified()) {
            return true;
            ...
            this.appDocumentViewer.getFile().pipe(
                ...
                tap(async (data: any) => {
                    if (this.functions.empty(data.contentView) && this.indexingForm.mandatoryFile) {
                        ...
                        if (formatdatas.destination === this.currentResourceInformations.destination) {
                            sameDestination = true;
                        }
                        // END EDISSYUM - NCH01
                        await this.indexingForm.saveData(!sameDestination); // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut - ajout de sameDestination dans la fonction
                        if (!this.detailMode) {
                            await this.getActions();
                        }
                        setTimeout(() => {
                            this.loadResource(false);
                            this.changedIndexingFormModel = false; // EDISSYUM - NCH01 Amélioration de l'écran de qualification pour récupérer les données par défaut
                        }, 400);
                    }
                })
            ).subscribe();
