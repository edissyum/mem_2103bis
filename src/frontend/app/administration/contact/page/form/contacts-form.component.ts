...
export class ContactsFormComponent implements OnInit {
    ...
    readOnlyMode: boolean = false; // EDISSYUM - NCH01 Rajout de la possibilité de visualiser les contacts sans droits d'administration
    privateContact: boolean = false; // EDISSYUM - NCH01 Rajout de la confidentialité des contacts
    ...
    constructor(
        ...
        public headerService: HeaderService, // EDISSYUM - NCH01 Rajout de la confidentialité des contacts
    ...
    ngOnInit(): void {
        ...
        if (!adminContact && !this.router.url.includes('/indexing/') && !this.router.url.includes('/process/')) {
        ...
        if (this.contactId === null) {
            ...
            this.http.get('../rest/contactsParameters').pipe(
                ...
                tap((data) => {
                    ...
                    this.initConfidentiality(); // EDISSYUM - NCH01 Rajout de la possibilité de visualiser les contacts sans droits d'administration
                }),
    ...
    isConfidential(field) {
        let userEntityAllowed = true;
        let requiredPrivilege;
        let hiddenFields = [];
        this.basicHiddenFields.forEach((hiddenField: any) => {
        ...
        this.advancedHiddenFields.forEach((hiddenField: any) => {
            if (hiddenField === field) {
        ...
        return this.privateContact && (!requiredPrivilege || !userEntityAllowed) && hiddenFields.includes(field);
    }
    ...
    initConfidentiality() {
        this.http.get('../rest/parameters/contactsConfidentiality').pipe(
            tap((data: any) => {
                const customFields = [];
                this.contactForm.filter((field: any) => field.id.includes('customField_')).forEach((element: any) => {
                    customFields.push({'id': element.id.replace('customField_', ''), 'value': element.control.value});
                });

                this.privateContact = false;
                if (data.parameter) {
                    const param = JSON.parse(data.parameter.param_value_string);
                    ...
                    customFields.forEach((customField: any) => {
                        if ((parseInt(customField.id) === parseInt(this.basicCustomId) || parseInt(customField.id) === parseInt(this.advancedCustomId)) && customField.value === 'Oui') {
                            this.privateContact = true;
                            if (parseInt(customField.id) === parseInt(this.basicCustomId)) {
                            ...
                            this.contactForm.forEach((field: any) => {
                                let fieldId = field.id.includes('address') ? 'address' : field.id;
                                if (fieldId.includes('customField_')) {
                                    fieldId = fieldId.replace('customField_', 'custom_');
                                }

                                if (this.isConfidential(fieldId) && this.readOnlyMode) {
                                    field.control.setValue('Coordonnées confidentielles');
                                }
                            });
                        }
                    });
    ...
    setContactCustomData(data: any) {
        ...
        Object.keys(data.customFields).forEach(element => {
            ...
            if (!this.isEmptyValue(data.customFields[element]) && indexField > -1) {
                ...
                console.log('element', element, data.customFields[element])
