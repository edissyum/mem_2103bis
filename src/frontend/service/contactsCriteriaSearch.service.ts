...
@Injectable()
export class ContactsCriteriaSearchService {
    ...
    initListsProperties(userId: number) {
        ...
        const crit = JSON.parse(sessionStorage.getItem('contatsCriteriaSearch_' + userId));

        if (crit !== null) {
            this.listsProperties = JSON.parse(sessionStorage.getItem('contatsCriteriaSearch_' + userId));
        } else {
    ...
    saveListsProperties() {
        sessionStorage.setItem('contatsCriteriaSearch_' + this.headerService.user.id, JSON.stringify(this.listsProperties));
    }
