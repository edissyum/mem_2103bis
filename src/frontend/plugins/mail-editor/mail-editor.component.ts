...
import { MatCheckboxChange } from '@angular/material/checkbox'; // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
...
export class MailEditorComponent implements OnInit, OnDestroy {
    ...
    receivedReceipt: boolean = false; // EDISSYUM - NCH01 Ajout d'un accusé de lecture
    ...
    checkBoxAttachments: any       = new FormControl();
    hasError: boolean   = false;
    errorMessage: string    = '';
    nextcloudFieldsFilled: boolean   = false;
    sentWithNextcloud: boolean = false; // EDISSYUM - AMO01 - Rajout d'une option -> mettre PJ dans mail via liens éphémères -> FIX BODY REPLACED BY [PJ_LINKS] AND CHECK CHECKBOX IF SENT VIA NEXTCLOUD
    attachmentsHostsConf: any = {
        nextcloud: {
            byDefault: new FormControl(false),
            url: new FormControl(''),
            username: new FormControl(''),
            password: new FormControl(''),
            folderNextcloud : new FormControl(''),
            urlExpirationDate : new FormControl(''),
            textAddedAboveURLS : new FormControl('Pièces Jointes :'),
        }
    };
    attachmentsHostsEnabled = [];
    // END EDISSYUM - AMO01-FIX - Rajout d'une option -> mettre PJ dans mail via liens éphémères
    constructor(
    ...
    async ngOnInit() {
        ...
        // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
        await this.getAttachmentsHostsConfiguration();
        this.nextcloudFieldsFilled = (this.attachmentsHostsConf['nextcloud'].username.value !== '' &&  this.attachmentsHostsConf['nextcloud'].password.value !== '' &&  this.attachmentsHostsConf['nextcloud'].url.value !== '');
        if (this.nextcloudFieldsFilled) {
            await this.checkNextcloudConnection();
        } else {
            this.hasError = true;
        }

        if (this.emailContent) {
            this.sentWithNextcloud = this.emailContent.includes('<a title=\'CTRL');
        }
    ...
    checkNextcloudConnection() {
        this.http.get('../rest/parameters/nextcloud/check').pipe(
            catchError(() => {
                this.hasError = true;
                return of(false);
            })
    ...
    getAttachmentsHostsConfiguration() {
        return new Promise((resolve, reject) => {
            this.http.get('../rest/configurations/admin_attachments_hosts').pipe(
                map((data: any) => data.configuration.value),
                tap((data: any) => {
                    if (data) {
                        Object.keys(data).forEach(confId => {
                            this.attachmentsHostsEnabled.push(confId);
                            Object.keys(data[confId]).forEach(itemId => {
                                if (!this.functions.empty(this.attachmentsHostsConf[confId][itemId])) {
                                    this.attachmentsHostsConf[confId][itemId].setValue(data[confId][itemId]);
                                }
                            });
                            if ((this.attachmentsHostsConf['nextcloud']['byDefault']['value']) && (!this.readonly) && (!this.hasError) && ((this.emailAttach.attachments && this.emailAttach.attachments.length > 0) || (this.emailAttach.document && this.emailAttach.document['isLinked']) || (this.emailAttach.notes && this.emailAttach.notes.length > 0) || (this.emailAttach.summarySheet && this.emailAttach.summarySheet.length > 0)))  {
                                this.emailContent = '<h5 class="PJ_LINKS">[PJ_LINKS]</h5>';
                            }
                        });
                    }
                    resolve(true);
        ...
        return new Promise((resolve) => {
            this.http.get(`../rest/emails/${emailId}`).pipe(
                tap(async (data: any) => {
                    ...
                    // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
                    if (this.emailContent && this.emailContent.includes('<a title=\'CTRL')) {
                        this.checkBoxAttachments.setValue(true);
                    }
                    // END EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
        ...
        if (type === 'document') {
            ...
            if (this.emailAttach[type].filter((attach: any) => attach.id === item.id).length === 0) {
                this.emailAttach[type].push({
                    ...
                    original: mode !== 'pdf',
                    size: mode === 'pdf' && item.convertedDocument ? item.convertedDocument.size : item.size // EDISSYUM - NCH01 Fix pour éviter une erreur lors de l'envoi de mail
                });
            }
        }

        // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
        if ((tinymce.get('emailSignature') !== null) && this.attachmentsHostsConf['nextcloud']['byDefault']['value'] && !this.hasError && this.nextcloudFieldsFilled && ((this.emailAttach.attachments.length > 0) || (this.emailAttach.document.isLinked) || (this.emailAttach.notes.length > 0) || (this.emailAttach.summarySheet.length > 0) )) {
            const current_emailContent = tinymce.get('emailSignature').getContent();
            tinymce.get('emailSignature').setContent('<h5 class="PJ_LINKS">[PJ_LINKS]</h5>' + current_emailContent);
        }
        // END EDISSYUM - AMO01
    }
        ...
        if (item.type === 'contactGroup') {
            this.http.get(`../rest/contactsGroups/${item.id}/correspondents?limit=none`).pipe(
                map((data: any) => {
                    ...
                    const tmpData = data; // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre

                    data = data.correspondents.filter((contact: any) => !this.functions.empty(contact.email)).map((contact: any) => ({
                        id: contact.id, // EDISSYUM - NCH01 Rajout de la possibilité de link un groupement de correspondants dans un autre
                        label: contact.name,
                        email: contact.email
                    }));
                    ...
                    if (data.length !== tmpData.length) {
                        tmpData.correspondents.forEach((contact: any, cpt: number) => {
                            if (contact.type !== 'contactGroup') {
                                data.forEach((contact_bis) => {
                                    if (contact_bis.id === contact.id) {
                                        tmpData.correspondents.splice(cpt, 1);
                                    }
                                });
                            }
                        });
                        this.setContactRecursively(tmpData, type);
                    }
                    // END EDISSYUM - NCH01
                    return data;
                }),
    ...
    setContactRecursively(contacts: any, type) {
        contacts.correspondents.forEach((contact: any) => {
            if (contact.type === 'contactGroup') {
                ...
                if (!this.functions.empty(contact.email)) {
    ...
    async createSummarySheet() {
        // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
        if (tinymce.get('emailSignature') !== null) {
            const current_emailContent = tinymce.get('emailSignature').getContent();
            const regex = /<h5.*h5>/g;
            const modified_current_emailContent = tinymce.get('emailSignature').getContent().replaceAll(regex, '');
            tinymce.get('emailSignature').setContent('<h5 class="PJ_LINKS">[PJ_LINKS]</h5>' + modified_current_emailContent);
        }
        // END EDISSYUM - AMO01
        return new Promise(resolve => {
    ...
    formatEmail() {
        ...
        return {
            document: objAttach,
            sender: formatSender,
            receivedReceipt: this.receivedReceipt, // EDISSYUM - NCH01 Ajout d'un accusé de lecture
            pjLinkChecked : this.checkBoxAttachments.value, // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
    ...
    removeAttachMail(index: number, type: string) {
        ...
        // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
        if ((this.emailAttach.attachments.length <= 0) && (!this.emailAttach.document.isLinked) &&
            (this.emailAttach.notes.length <= 0) && (this.emailAttach.summarySheet.length <= 0)) {
            const regex = /<h5.*h5>/g;
            const modified_current_emailContent = tinymce.get('emailSignature').getContent().replaceAll(regex, '');
            tinymce.get('emailSignature').setContent(modified_current_emailContent);
        }
        // END EDISSYUM - AMO01
    }
    ...
    // EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
    changeCheckBoxValueAttachments(event: MatCheckboxChange) {
        const current_emailContent = tinymce.get('emailSignature').getContent();
        if (event['checked'] == false) { // si les liens sont déjà dans le corps du mail, je les supprime.
            const regex = /<h5.*h5>/g;
            const modified_current_emailContent = tinymce.get('emailSignature').getContent().replaceAll(regex, '');
            tinymce.get('emailSignature').setContent(modified_current_emailContent);
        } else { // sinon, je les append au début du mail.
            tinymce.get('emailSignature').setContent('<h5 class="PJ_LINKS">[PJ_LINKS]</h5>' + current_emailContent);
        }
    };
    // END EDISSYUM - AMO01 Rajout d'une option -> mettre PJ dans mail via liens éphémères
