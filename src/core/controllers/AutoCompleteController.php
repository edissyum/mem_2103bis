...
class AutoCompleteController
{
    ...
    public static function getCorrespondents(Request $request, Response $response)
    {
        ...
        // EDISSYUM - NCH01 Augmentation du nombre de résultats pour les destinataires / expéditeurs
        $maxItems = ParameterModel::getById(['id' => 'CorrespondantMaxItems']);
        if (count($maxItems) > 0) {
            $limit = $maxItems['param_value_int'];
        } else {
            $limit = self::TINY_LIMIT;
        }
        // END EDISSYUM - NCH01
        ...
        if (empty($queryParams['noContacts'])) {
            ...
            $contacts = ContactModel::get([
                'select'    => ['id', 'email', 'custom_fields'], // EDISSYUM - NCH01 Rajout de la confidentialité des contacts | ajout de custom_fields
                'where'     => $requestData['where'],
                'data'      => $requestData['data'],
