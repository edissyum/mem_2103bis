...
class CoreConfigModel
{
    public static function getCustomId()
    {
        ...
        // EDISSYUM - PyB01 Patch pour fonctionner sur les VPS ou pour fonctionner avec un custom mentionné dans le vhost
        $custom_name = $_SERVER['HTTP_HOST'];
        if(file_exists($_SESSION['config']['corepath'] . 'custom' . DIRECTORY_SEPARATOR . $custom_name))
            return (string) $custom_name;

        if(isset($_SERVER['CUSTOM_MAARCH']))
            return $_SERVER['CUSTOM_MAARCH'];
        // END EDISSYUM - Pyb01
    ...
    public static function getApplicationName()
    {
        ...
        $applicationName = 'MEM Courrier'; // EDISSYUM - NCH01 Changement du nom d'application
        return $applicationName;
    }
